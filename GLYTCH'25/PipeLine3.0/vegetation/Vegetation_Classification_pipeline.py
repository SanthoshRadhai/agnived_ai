...
# NOTE: BigEarthNetv2_0_ImageClassifier is imported inside load_reben_model()
# after the sys.path is configured properly
...
def load_reben_model(device: torch.device) -> torch.nn.Module:
    ...
    print(f"[DEBUG] Looking for reBEN at: {local_reben}")
    print(f"[DEBUG] Candidate reBEN paths: {candidate_paths}")
    
    if not candidate_paths:
        raise SystemExit(
            f"❌ reBEN code not found.\n"
    ...
    print(f"[DEBUG] Adding to sys.path: {reben_dir}")
    sys.path.insert(0, str(reben_dir))
    
    # Also add the parent directory so we can import reben_publication as a package
    parent_dir = reben_dir.parent
    if str(parent_dir) not in sys.path:
        sys.path.insert(0, str(parent_dir))
        print(f"[DEBUG] Also added parent to sys.path: {parent_dir}")
    ...
    try:
        ...
        print("[DEBUG] Successfully imported BigEarthNetv2_0_ImageClassifier")
    except ImportError as exc:
        print(f"[ERROR] Failed to import reBEN modules")
        print(f"[ERROR] reben_dir: {reben_dir}")
        print(f"[ERROR] sys.path: {sys.path[:5]}")  # Print first 5 paths
        import traceback
        traceback.print_exc()
...
if __name__ == "__main__":
    ...
    mask_path = Path(
        r"E:\6thSem\GLYTCH'25\PipeLine3.0\LandcoverResults\vegetation_mask.tif"
    )
